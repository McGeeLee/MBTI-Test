...
class AppStrings {
  ...
  String get splashBody => switch (locale) {
    AppLocale.vi =>
      'Ứng dụng MBTI mobile với dữ liệu đa ngôn ngữ và trải nghiệm làm bài mượt hơn.',
    AppLocale.en =>
      'A mobile MBTI app with multilingual content and a cleaner test experience.',
  ...
  String get heroBody => switch (locale) {
    AppLocale.vi =>
      'Giao diện trẻ trung hơn, dữ liệu nhiều ngôn ngữ hơn và trải nghiệm làm bài tối ưu cho mobile.',
    AppLocale.en =>
      'A brighter mobile-first interface with multilingual data and faster test flows.',
  ...
  String get homeSectionSubtitle => switch (locale) {
    AppLocale.vi =>
      'Ba phiên bản với nhịp độ khác nhau. Bản tiêu chuẩn vẫn là lựa chọn cân bằng nhất.',
    AppLocale.en =>
      'Three versions with different depth and pacing. Standard is still the most balanced choice.',
  ...
  String get statTwoBody => switch (locale) {
    ...
    AppLocale.en =>
      'Quick, standard, and full versions for different time budgets.',
  ...
  String resumePromptBody(String versionTitle) => switch (locale) {
    AppLocale.vi =>
      'Bài $versionTitle đã được lưu cục bộ. Bạn có thể tiếp tục từ vị trí cũ hoặc reset lại.',
    AppLocale.en =>
      'Your $versionTitle test is saved locally. You can continue from where you left off or reset it.',
  ...
  String questionCountLabel(int count) => switch (locale) {
    AppLocale.vi => '$count câu',
    AppLocale.en => '$count questions',
    AppLocale.ko => '$count문항',
    AppLocale.ja => '$count問',
    AppLocale.zh => '$count 题',
  };
  ...
  String get librarySubtitle => switch (locale) {
    AppLocale.vi =>
      'Tra cứu nhanh từng nhóm, đọc mô tả ngắn và mở hồ sơ chi tiết ngay trong app.',
    AppLocale.en =>
      'Browse each type quickly, read a short summary, and open the full profile in the app.',
  ...
  String get historySubtitle => switch (locale) {
    AppLocale.vi =>
      'Toàn bộ kết quả được lưu ngay trên thiết bị để bạn mở lại, so sánh hoặc xoá từng mục.',
    AppLocale.en =>
      'All results are saved on the device so you can reopen, compare, or delete them anytime.',
  ...
  String get noHistoryBody => switch (locale) {
    AppLocale.vi =>
      'Làm một bài test từ trang chủ để tạo kết quả đầu tiên và xem lịch sử tại đây.',
    AppLocale.en =>
      'Take a test from the home screen to create your first result and see it here.',
  ...
  String get aboutSubtitle => switch (locale) {
    AppLocale.vi =>
      'MBTI là khung tham chiếu phổ biến để mô tả xu hướng nhận thức, ra quyết định và cách bạn tổ chức cuộc sống.',
    AppLocale.en =>
      'MBTI is a popular framework for describing how people process information, decide, and structure life.',
    AppLocale.ko =>
      'MBTI는 사람들이 정보를 인식하고 의사결정을 내리며 삶을 구조화하는 방식을 설명하는 데 널리 쓰이는 프레임워크입니다.',
  ...
  String get aboutUsageBody => switch (locale) {
    AppLocale.vi =>
      'Ứng dụng này là công cụ tự khám phá không chính thức. Kết quả chỉ nên được xem như tài liệu tham khảo để hiểu bản thân, không thay thế tư vấn tâm lý hoặc đánh giá chuyên môn.',
    AppLocale.en =>
      'This app is an informal self-discovery tool. Results should be treated as reference material, not as a replacement for professional psychological advice or assessment.',
    AppLocale.ko =>
      '이 앱은 비공식 자기 탐색 도구입니다. 결과는 참고용으로만 활용해야 하며 전문 심리 상담이나 평가를 대체하지 않습니다.',
    AppLocale.ja =>
      'このアプリは非公式な自己理解ツールです。結果は参考情報として扱い、専門的な心理相談や評価の代替とは考えないでください。',
  ...
  String get settingsLanguageSubtitle => switch (locale) {
    AppLocale.vi =>
      'Vietnamese mặc định, kèm English, Korean, Japanese và Chinese.',
    AppLocale.en =>
      'Vietnamese by default, with English, Korean, Japanese, and Chinese available.',
  ...
  String get privacyBody => switch (locale) {
    AppLocale.vi =>
      'Kết quả và tiến độ làm bài được lưu hoàn toàn trên thiết bị của bạn và không được gửi lên bất kỳ máy chủ nào. Ứng dụng không hiển thị quảng cáo, không thu thập dữ liệu cá nhân và không yêu cầu đăng nhập. Bạn có thể xoá dữ liệu bất cứ lúc nào trong mục Lịch sử.',
    AppLocale.en =>
      'Your results and test progress are stored entirely on your device and are never sent to any server. The app shows no ads, collects no personal data, and requires no sign-in. You can clear your data anytime from the History tab.',
    AppLocale.ko =>
      '결과와 진행 상황은 전적으로 기기에 저장되며 어떤 서버로도 전송되지 않습니다. 이 앱은 광고를 표시하지 않고 개인정보를 수집하지 않으며 로그인이 필요하지 않습니다. 기록 탭에서 언제든지 데이터를 삭제할 수 있습니다.',
    AppLocale.ja =>
      '結果と進行状況はすべて端末に保存され、いかなるサーバーにも送信されません。本アプリは広告を表示せず、個人データを収集せず、ログインも不要です。履歴タブからいつでもデータを削除できます。',
    AppLocale.zh =>
      '你的结果和测试进度完全保存在设备上，不会发送到任何服务器。应用不展示广告、不收集个人数据，也无需登录。你可以随时在历史记录中清除数据。',
  };
